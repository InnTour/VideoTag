    ...
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{
        ...
        background: [
          ...
          'linear-gradient(to top, rgba(8,8,12,0.88) 0%, transparent 55%)',
        ].join(', '),
      }} />
      <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse at center, transparent 48%, rgba(0,0,0,0.75) 100%)' }} />

      {flashOp > 0 && <div style={{ position: 'absolute', inset: 0, background: COLORS.travertino, opacity: flashOp }} />}
      ...
      <div style={{ position: 'absolute', left: 72, bottom: 230, opacity: interpolate(title, [0, 1], [0, 1]), transform: `translateY(${interpolate(title, [0, 1], [40, 0])}px)` }}>
        ...
        <div style={{
          ...
          Chiesa della<br /><span style={{ color: COLORS.oroAntico }}>Consolazione</span>
        </div>
        <div style={{
          position: 'absolute', right: -120, top: -30,
          fontFamily: 'Playfair Display, serif', fontSize: 150, fontWeight: 700,
          color: COLORS.oroAntico, opacity: 0.06, pointerEvents: 'none',
        }}>A3.01</div>
      </div>
